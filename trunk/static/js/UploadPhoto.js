...
var uploadPhoto = function(e){
	...
	$("#upload_img").load(function(){
		var allowExtention=".jpg,.bmp,.gif,.png";//允许上传文件的后缀名
              ...
              setTimeout(imgEventInit(".adjust_frame","#upload_img"),2000);
              // stopProgress();
              $(".upload_txt").css("display","none");
              $(".uploadIntro_txt").css("display","none");
              $(".page2_photoBtn").addClass("f-dn");
              $(".page2_repick").removeClass("f-dn");
              $(".page2_doneBtn").removeClass("f-dn");
              $(".page2_grid").removeClass("f-dn");
              
              
              // $(".u-bg").css('background-image','url(img/adjust_bg.jpg)');
              // $(".adjust_frame").removeClass("f-dn");
              // $(".eye_frame").removeClass("f-dn");

              $(".upload_photo").each(function(index, el) {
                $(this).height($(this).width());
                // $(this).css('border-radius',$(this).width()/2);
                $(this).css('-webkit-border-radius',$(this).width()/2);
              });
...
function imgEventInit(divId,imgId){
    ...
    var el = photo[0];
    var scaleX,scaleY;
    console.log(content.width() + ', ' + photo.width())
...
function picBeautify(imgId){
	...
	var img = $(imgId)[0];
	var shieldW = $(".adjust_frame").width();
	var shieldH = $(".adjust_frame").height();
	var content = {
		width:shieldW,
		height:shieldH
	};
